...
class Boxalino_Frontend_Helper_P13n_Adapter
{
    ...
    public function addFilterHierarchy($field, $hierarchyId, $hierarchy, $lang = null)
    {
        ...
        if ($lang) {
            $filter->fieldName = $field . '_' . substr($this->scopeConfig->getValue('general/locale/code',$this->scopeStore), 0, 2);
        } else {
    ...
    {
        $searchConfig = $this->scopeConfig->getValue('Boxalino_General/search',$this->scopeStore);
        if ($fields == null) {
        ...
        $searchQuery->language = substr($this->scopeConfig->getValue('general/locale/code',$this->scopeStore), 0, 2);
        ...
        $config = $this->scopeConfig->getValue('Boxalino_General/autocomplete_extra',$this->scopeStore);
        if ($config['enabled']) {
        ...
        $autocompleteQuery->language = substr($this->scopeConfig->getValue('general/locale/code',$this->scopeStore), 0, 2);
        ...
        foreach ($this->autocompleteResponse->hits as $hit) {
            ...
            if ($config['enabled']) {
                $tmp['facets'] = array_slice($this->getFacetLeafs($hit->searchResult->facetResponses[0]->values, $hit, $config),
                    0, $this->scopeConfig->getValue('Boxalino_General/autocomplete_extra/items',$this->scopeStore));
            }
    ...
    private function filterByVisibleProducts()
    {
        ...
        $filter->stringValues = array(\Magento\Catalog\Model\Product\Visibility::VISIBILITY_NOT_VISIBLE,
    ...
    private function filterByStatusProducts()
    {
        ...
        $filter->stringValues = array(\Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_ENABLED);
        return $filter;
    }
    ...
    public function getAutocompleteProducts($facets, $map = null, $fields = null)
    {
        ...
        $extraConfig = $this->scopeConfig->getValue('Boxalino_General/autocomplete_extra',$this->scopeStore);
    ...
    public function search()
    {
        ...
        if ($this->scopeConfig->getValue('Boxalino_General/search_relaxation/enabled',$this->scopeStore) == 1) {
    ...
    private function prepareFacets()
    {
        ...
        $normalFilters = array();
        $topFilters = array();
        $enableLeftFilters = $this->scopeConfig->getValue('Boxalino_General/filter/left_filters_enable',$this->scopeStore);
        $enableTopFilters = $this->scopeConfig->getValue('Boxalino_General/filter/top_filters_enable',$this->scopeStore);

        if ($enableLeftFilters == 1) {
            $normalFilters = explode(',', $this->scopeConfig->getValue('Boxalino_General/filter/left_filters_normal',$this->scopeStore));
        }
        if ($enableTopFilters == 1) {
            $topFilters = explode(',', $this->scopeConfig->getValue('Boxalino_General/filter/top_filters',$this->scopeStore));
        }
    ...
    public function getEntitiesIds()
    {
        ...
        foreach ($response->variants as $variant) {
            ...
            foreach ($searchResult->hits as $item) {
                $result[] = $item->values[$this->scopeConfig->getValue('Boxalino_General/search/entity_id',$this->scopeStore)][0];
            }
        }
    ...
    public function prepareAdditionalDataFromP13n()
    {
        ...
        $this->session->setData('boxalino_additional_data', $result);
    }
